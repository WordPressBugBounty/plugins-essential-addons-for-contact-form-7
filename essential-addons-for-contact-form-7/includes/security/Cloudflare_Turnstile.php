...
class Cloudflare_Turnstile {
    ...
    public function __construct() {
        ...
        add_filter('wpcf7_validate_cloudflare_turnstile', array($this, 'validate'), 10, 2);
    }
    ...
    public function add_data_handler() {
        if (function_exists('wpcf7_add_form_tag')) {
            wpcf7_add_form_tag(
                array('cloudflare_turnstile'),
                array($this, 'render_cloudflare_turnstile'),
                array(
                    'name-attr'    => true,
                    'do-not-store' => true,
                    'not-for-mail' => true,
     ...
     * @return void
     */
    public function render_cloudflare_turnstile($tag) {
...
?>
        <div class='cf-turnstile' data-sitekey="<?php echo eacf7_get_settings('cloudflareSiteKey'); ?>" data-theme="<?php echo esc_attr( $theme ); ?>"></div>
    ...
    public function add_tag_generator() {
        ...
        if (version_compare(WPCF7_VERSION, '6.0', '>=')) {
            $tag_generator->add(
                ...
                __('Cloudflare Turnstile', 'essential-addons-for-contact-form-7'),
        ...
        <header class="description-box">
            <div class="eacf7-notice eacf7-notice-info">
                <p>
                    ...
                    <?php
                    echo sprintf(
                        __('Not sure how to set this? Check our documentation on <a href="%1$s" target="_blank">Cloudflare Turnstile</a>', 'essential-addons-for-contact-form-7'),
                        'https://softlabbd.com/docs/cloudflare_turnstile/'
                    );
                    ?>
                </p>
            </div>
        ...
        <div class="control-box">
            ...
            $tgg->print('field_type', array(
                ...
                'select_options' => array(
                    'cloudflare_turnstile' => esc_html__('Cloudflare Turnstile', 'essential-addons-for-contact-form-7'),
                ),
            ));
                ...
                <table class="form-table">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <input type="text" name="class" class="class-attributes oneline option" id="class-attributes" />
                            </td>
                        </tr>
                        <tr>
                            <th scope="row">
...
<?php
    ...
    public function validate($result, $tag) {
        ...
        $cf_turnstile_response = isset($_POST['cf-turnstile-response']) ? sanitize_text_field($_POST['cf-turnstile-response']) : '';

        if (empty($cf_turnstile_response)) {
            $result->invalidate($tag, esc_html__('Please complete the Cloudflare Turnstile verification.', 'essential-addons-for-contact-form-7'));
            return $result;
        }

        $secret_key = eacf7_get_settings('cloudflareSecretKey');

        if (empty($secret_key)) {
            $result->invalidate($tag, esc_html__('Cloudflare Turnstile secrect key error.', 'essential-addons-for-contact-form-7'));
            return $result;
        }
        ...
        if (!$result_data['success'] && empty($cf_turnstile_response)) {
            $result->invalidate($tag, esc_html__('Cloudflare Turnstile verification failed. Please try again.', 'essential-addons-for-contact-form-7'));
        }
    ...
    /**
     * @return Cloudflare_Turnstile|null
     */
